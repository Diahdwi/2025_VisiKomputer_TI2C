...
# ===== PUSH-UP RATIO FUNCTION =====
def ratio_pushup(lm):
    # 11=shoulderL, 15=wristL, 23=hipL
    shoulder = np.array(lm[11][1:3])
    wrist = np.array(lm[15][1:3])
    hip = np.array(lm[23][1:3])

    return np.linalg.norm(shoulder - wrist) / (np.linalg.norm(shoulder - hip) + 1e-8)
...
while True:
    ...
    flag = None  # sementara state dari frame ini

    if lmList:
        if MODE == "squat":
            # Compute knee angles kiri & kanan
            ...
            cv2.putText(img, f"Knee: {knee_angle:5.1f}", (20, 70),
